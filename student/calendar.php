    ...
    try {
        ...
        if (isset($_POST['export_ical'])) {
            ...
            $events = dbFetchAll("
                ...
                FROM events
                WHERE is_active = 1 AND DATE(event_date) BETWEEN ? AND LAST_DAY(?)
...
// Build query
$whereConditions = ["event_date BETWEEN ? AND ?", "is_active = 1"];
...
$categories = dbFetchAll("SELECT DISTINCT category FROM events WHERE is_active = 1 AND category IS NOT NULL ORDER BY category");
...
$importantDates = dbFetchAll("
    ...
    FROM (
        ...
        FROM events
        WHERE registration_deadline > NOW() AND is_active = 1
        ...
        FROM events
        WHERE fee > 0 AND event_date > DATE_ADD(NOW(), INTERVAL 1 DAY) AND is_active = 1
